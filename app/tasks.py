...
from app.models import Order, Ticket, Notification, KST
...
def auto_confirm_purchases():
    ...
    with scheduler.app.app_context():
        print(f"[{datetime.now(KST)}] 자동 구매확정 스케줄러 실행 중...")
        
        # 7일 전의 시간 계산 (KST 타임존 적용)
...
@scheduler.task('interval', id='expire_tickets_job', minutes=10)
